...
int countOneChildNodes(BTNode *node) {
    ...
    // 왼쪽 서브트리에서 자식이 하나인 노드 수 계산
    int leftCount = countOneChildNodes(node->left);

    // 오른쪽 서브트리에서 자식이 하나인 노드 수 계산
    int rightCount = countOneChildNodes(node->right);

    // 현재 노드가 자식 하나만 가지고 있는지 확인
    int hasOnlyOneCount = (node->left == NULL) != (node->right == NULL);

    // 현재 노드 결과 + 왼쪽 + 오른쪽 결과 모두 더해서 반환
    return hasOnlyOneCount + leftCount + rightCount;
}
